...
member_data_frame <- as.data.frame(DBI::dbFetch(rs,n=-1))
DBI::dbClearResult(rs)
rs <- DBI::dbSendQuery(my_database,"select * from appointments")
appointments_data_frame <- as.data.frame(DBI::dbFetch(rs,n=-1))
DBI::dbClearResult(rs)
rs <- DBI::dbSendQuery(my_database,"select * from voting")
voting_data_frame <- as.data.frame(DBI::dbFetch(rs,n=-1))
DBI::dbClearResult(rs)
...
test_that("inputs are correct create database function.", {
  expected_obj <- TRUE
  #actual_obj <- create_database(appointments_data_frame,member_data_frame,voting_data_frame)
